...
class ModelSamplingWaifuTan(nn.Module):
    sigmas: Tensor
    log_sigmas: Tensor
...
class ModelSamplingCosXLTC(nn.Module):
    ...
    def timestep(self, sigma):
        """actually c_noise - only timestep for SDXL"""
        # COSXL
        c_noise = 0.25 * sigma.log()
        return c_noise
    ...
    def sigma(self, timestep):
        """
        Warning: sigma() is not exact here, schedule reinterpretation methods that use it may be inaccurate.
        """
        if math.isclose(timestep, self.timestep(self.sigma_min), abs_tol=1e-6):
            return self.sigma_min
        elif math.isclose(timestep, self.timestep(self.sigma_max), abs_tol=1e-6):
...
class ModelSamplingWaifuEDM:
    ...
    def patch(
        ...
        match schedule:
            case "Tan":
                use_tan = True
                model_class = ModelSamplingWaifuTan
            case "XLTC":
                use_tan = True
...
NODE_CLASS_MAPPINGS = {
    "ModelSamplingWaifuEDM": ModelSamplingWaifuEDM,
}
